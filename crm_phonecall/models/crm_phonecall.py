...
class CrmPhonecall(models.Model):
    ...
    state = fields.Selection(
        [#('open', 'Confirmed'),
         ('pending', 'Pending'),
         ('incorrect', 'Incorrect Contact'),
         ('nointerestcamp', 'Uninterested in Campaign'),
         ('nointerestcomp', 'Uninterested in Company'),
         #('cancel', 'Cancelled'),
         #('lost', 'Lost'),
         ('done', 'Opportunity')
         ],
    ...
    def convert_opportunity(self, opportunity_summary=False, partner_id=False,
        ...
        stage_id = []
        default_stage = self.env['crm.stage'].search([('name','=','S20-Lead')])
        if default_stage:
